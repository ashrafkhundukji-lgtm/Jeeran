...
// Plain BALOO, not displayFont(locale) — businessName is shop-typed content,
// not locale-driven copy (same reasoning as OfferPageView's own constant).
    ...
    <main dir={dir} className="min-h-screen bg-[#FFF8EC] pb-24 md:pb-10">
      ...
      <div className="mx-auto max-w-3xl px-5 pt-[22px]">
        ...
        <div className="mb-5 flex items-center gap-2.5">
          <h1 className={`${BALOO} text-[27px] font-black leading-[1.15] tracking-[-0.01em]`}>{businessName}</h1>
          <LevelBadge level={promotionLevel} variant="chip" locale={locale} />
        </div>
        <p className="-mt-4 mb-5 text-[13px] text-[#8A76BE]">
        ...
        {/* Credits hero card */}
        <div
          className="mb-3.5 rounded-[22px] p-[22px] text-white shadow-[0_14px_28px_-14px_rgba(46,16,101,0.55)]"
          style={{
            background: isSubscriptionActive
              ? 'linear-gradient(135deg,#2E1065,#4C2A98)'
        ...
        >
          ...
          <a
            ...
            className="mt-3.5 inline-block rounded-full bg-white px-4 py-[9px] text-[13px] font-semibold text-[#2E1065]"
          >
            {copy.topUpBalance}
          </a>
        </div>
        ...
        <div className="mb-[26px]">
          <div className="mb-2.5 flex items-center justify-between">
            <h2 className="text-[13px] font-semibold tracking-[0.06em] text-[#8A76BE]">{copy.activeOfferHeading}</h2>
            <button type="button" onClick={openCampaignForm} className="text-[13px] font-semibold text-[#FF5A79]">
          ...
          {activeCampaign ? (
            <div className="rounded-[18px] border border-[#EDE3F7] bg-white p-3.5 transition-transform active:scale-[0.99]">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[12.5px] text-[#8A76BE]">
                ...
                <button
                  ...
                  className="relative h-7 w-[46px] shrink-0 rounded-full bg-[#FF5A79] p-[3px]"
                >
                  <span
              ...
              <div className="mt-3 border-t border-[#f2f2f2] pt-3">
                <a href={`/offers/${activeCampaign.id}`} className="flex items-center gap-1.5 text-[13px] font-medium text-[#2E1065]">
            ...
            <button
              ...
              className="w-full rounded-[18px] border border-dashed border-[#E3D6F5] p-5 text-center text-[14px] font-medium text-[#6B5A8C]"
            >
        ...
        {/* QR stand row */}
        <div className="mb-3 flex items-center gap-3 rounded-[18px] border border-[#EDE3F7] bg-white p-3.5">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[12.5px] text-[#8A76BE]">{copy.qrStandBody}</div>
          </div>
          <a
        ...
        <a
          ...
          className="mb-10 flex items-center gap-3 rounded-[18px] border border-[#EDE3F7] bg-white p-3.5"
        >
...
function StatTile({ value, label }: { value: number | string; label: string }) {
  return (
    <div className="rounded-[18px] border border-[#EDE3F7] bg-white p-4">
      <div className="text-[30px] font-bold leading-none">{value}</div>
      <div className="mt-[5px] text-[12.5px] text-[#8A76BE]">{label}</div>
    </div>
  )
}
