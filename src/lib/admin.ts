...
export async function getAdminOverview(): Promise<AdminOverview> {
  const [{ data: businesses, error: bizError }, { data: campaigns, error: campError }, { data: scans, error: scanError }] =
    await Promise.all([
      supabaseAdmin
        .from('businesses')
        .select('id, name, category, owner_id, latitude, longitude, ad_credits, is_subscription_active, created_at')
